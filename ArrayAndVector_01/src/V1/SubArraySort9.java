...
//		last element se chota nahi hona chayea 2nd last se
		if (index == arr.length - 1) {
			return arr[index] < arr[index - 1];
		}
//		middel element does not greater then next element se ya does not smaller then previous element
		return arr[index] > arr[index + 1] || arr[index] < arr[index - 1];
	}
//	aagar ye sab condition hai tb to outOfOrder hai element 
}
